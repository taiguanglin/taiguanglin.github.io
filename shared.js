...
(function () {
    ...
    if (hamburger && navMenu) {
        ...
        navMenu.querySelectorAll('.nav-link').forEach(function (link) {
            /* 「圖解」toggle 不關選單：手機版滑入選單內點擊時要先展開子選單，
             * 若掛上 closeMenu 會把整個面板收起，子選單永遠看不到。 */
    ...
    document.querySelectorAll('.nav-dropdown-toggle').forEach(function (toggle) {
        ...
        var dropdownMenu = dd ? dd.querySelector('.nav-dropdown-menu') : null;
        if (!dd || !dropdownMenu) return;
        ...
        toggle.addEventListener('click', function (e) {
            e.preventDefault();
            setDropdown(!dd.classList.contains('active'));
        });
    ...
    (function navActive() {
        ...
        if (file === 'infographic.html' || file === 'mindmap.html' ||
            file === 'wenda2_mindmap.html') {
            var item = null;
            graphicItems.forEach(function (a) {
                if (hrefOf(a).split('/').pop() === file) item = a;
            });
