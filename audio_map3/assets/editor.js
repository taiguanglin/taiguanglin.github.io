...
const MAP_BASE = './';
// 講經系列（audio_map3）：對外檔案名 = 系列 key，貼在王「系列」下拉。
const SERIES = [
    { key: 'ganen',        label: '感恩與講經' },
    { key: 'sishierzhang', label: '四十二章經' },
    { key: 'lengqie',      label: '楞伽經' },
    { key: 'liuzutanjing', label: '六祖壇經' },
    { key: 'lengyanjing',  label: '楞嚴經' },
];
const MONTHS = SERIES.map((s) => s.key);
const SERIES_LABEL = Object.fromEntries(SERIES.map((s) => [s.key, s.label]));
...
async function bootstrap() {
    els.monthSelect.innerHTML = MONTHS.map((m) => `<option value="${m}">${SERIES_LABEL[m] || m}</option>`).join('');
    bindEvents();
    setupMiniPlayer();
...
/** 存檔前把內部 sessions 模型轉回 audio_map3 的 系列/講次/段落 格式。 */
function serializeMapForSave(map) {
    return `${JSON.stringify(map2ToMap3(map), null, 2)}\n`;
}
...
function sessionItems() {
    ...
    for (const seg of session.segments || []) {
        const q = seg.q_text || '';
        // 講經段落：無「提問/回答」之分，整段即內容。標題顯示序號＋pid，
        // 正文（answer_body）顯示完整文字。
        items.push({
            kind: 'segment',
            item: seg,
            number: String(seg.index),
            title: `第 ${seg.index} 段 · ${seg.stable_key || ''}`,
        });
    }
    if (session.closing) {
...
// ---- audio_map3（系列/講次/段落）↔ audio_map2 內部（month/session/segment）轉換 ----
// 內部沿用 audio_map2 的 model（sessions[].segments[]），僅在載入／存檔時互轉：
//   段落 paragraph → segment（q_text = answer_text = text、stable_key = pid、
//   confidence = conf、status = confirmed? 'reviewed' : 'auto'、meta.lastPlayed = confirmed 以沿用「已聽=完成」機制）
function map3ToMap2(data) {
    const sessions = [];
    const lectures = data?.lectures || {};
    for (const n of Object.keys(lectures).sort((a, b) => Number(a) - Number(b))) {
        const lec = lectures[n];
        const segments = (lec.paragraphs || []).map((p, i) => {
            const seg = {
                index: i + 1,
                stable_key: p.pid,
                q_text: p.text || '',
                ...
                start_label: p.start != null ? secondsToTimecode(p.start) : null,
                end_label: p.end != null ? secondsToTimecode(p.end) : null,
        ...
        sessions.push({
            session_id: `${data.series}:${n}`,
            session_label: lec.title || `第 ${n} 講`,
            audio_file: lec.audio || '',
            reviewed: !!lec.reviewed,
            duration: lec.duration ?? null,
            segments,
        });
    }
    return { version: 1, series: data.series, book_number: data.book_number, sessions };
}

function map2ToMap3(map) {
    const lectures = {};
    for (const s of map?.sessions || []) {
        const [, n] = String(s.session_id || ':').split(':');
        lectures[n || s.session_id] = {
            audio: s.audio_file || '',
    ...
    return {
        series: map?.series || '',
        book_number: map?.book_number ?? null,
...
async function loadMonth(month, { forceRemote = false } = {}) {
    ...
    try {
        ...
        setStatus(`已載入 ${month}（${state.map.sessions?.length || 0} 講）`, 'ok');
    } catch (error) {
...
function sessionLabel(session) {
    return session?.session_label || session?.session_id || '';
}
...
function renderSessionList() {
    ...
    for (const session of sessions) {
        ...
        btn.innerHTML = `
            ...
            <div class="file-item-body">
                ...
                <span class="file-stats">✓${stats.matched} · ✗${stats.missing} · ${stats.total} 段 · 確認 ${stats.completed}</span>
            </div>
        `;
    ...
    if (!shown) {
        els.fileList.innerHTML = '<div class="empty-state">沒有講次資料</div>';
    }
}
...
async function saveCurrentMap({ force = false, reason = 'edit' } = {}) {
    ...
    try {
        ...
        const text = serializeMapForSave(payload);
        const message = reason === 'played'
            ? `Update audio_map3 confirm progress ${state.month}`
            : `Update audio_map3 ${state.month}`;
