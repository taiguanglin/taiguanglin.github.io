...
這份知識庫完整保存 2026-09-16 至 2026-09-17「mindmap.html 名詞查證與圖解工程」session 的可復用知識：查證方法、逐批發現與修正、證據行號、禁用術語、關鍵節點定稿、腳本與回歸基準。與本知識庫相關的產出：`books_knowledge.html`（坐禪與講經重點知識）、`books_knowledge_full.html`（坐禪與講經知識庫全檔）、`mindmap.html`（修行次第線區塊）。
...
5. review.html（間隔重複閃卡＋名詞測驗）← 已於後續移除
...
直接從 repo 的 mindmap.html 抽 `<script>` eval，不需 /tmp 快取。mm_harness 斷言盤點：SVG 標籤簡體×8、繁體不出現×5、跏趺坐×2、七處徵心存在×1、aria-label×1、detail 面板×5、深連結×3、EBOOK_LINKS×2、chips×2、axioms×4、b-truth×4、chart×3、ladder×7。

（`rv_harness.js` 隨 review.html 一併移除。）

重要：harness 讀的是從 html 抽出的腳本——mindmap.html 改完後直接重跑即可（自抽取版無此陷阱；舊 /tmp 版必須先重新 extract）。
...
- 規則 7（AI 免責聲明）：`infographic.html`、`mindmap.html`、`wenda2_knowledge.html`、`wenda2_mindmap.html`、`wenda2_knowledge_full.html` 及本次新增的 `books_knowledge.html`、`books_knowledge_full.html`、`session_knowledge.html` 都必須顯示「本頁圖解由 AI 生成，內容僅供參考，請以 Tai 師父原文教導為準。」
...
- 本次新增：`.mm-ladder-*`（mindmap 修行次第線）、`.bk-*`（books_knowledge 頁）；`.rv-*` 已隨 review.html 移除。
...
- 「圖解」下拉全站統一 3 項：名詞圖解／坐禪與講經名詞導覽／問答錄2 名詞導覽；導覽一律先落在檔案較小、載入最快的心智圖，按書重點／章節重點／全檔從心智圖往下一層進去。
- 導覽列頂層邏輯順序：首頁／禪師／入門路徑／著作／問答錄 2／圖解 ▾／實修故事／下載資料——認識（首頁→禪師→入門路徑）、兩大系列（著作→問答錄 2）與共用知識工具（圖解）相連、延伸內容（實修故事）、行動（下載）永遠最後。
- nav HTML 改由 `tool/site_chrome/sync.py` 統一寫回（shared.js 只加行為）；改導覽改 `ITEMS` 再重跑，不要逐頁手改。
...
| 建議 | 產出 | 狀態 |
|---|---|---|
| 閃卡＋測驗同頁 | review.html | 已落地（後續移除） |
| 修行次第線 | mindmap.html 新區塊 | 已落地 |
| 對比卡／跨書對照 | books_knowledge.html（按書分冊＋跨書名詞重複可見） | 已落地 |
| 未來：wenda2 57 節點接同一引擎 | 原 `window.tgl_review` API | 已取消（引擎隨頁移除） |
...
python3 tool/site_chrome/sync.py --check     # 共用導覽同步
python3 tool/site_chrome/check_site.py       # 站台不變量
node tool/site_chrome/dropdown_harness.js    # 下拉互動（38 頁）
node --check（改 mindmap.html 內嵌腳本後先跑）
```
...
| style.css | .bk-*、.mm-ladder-*、.knowledge-view-* 樣式 | 手編 |
| tool/books_knowledge/ | build.js＋README.md | 新工具 |
| tool/session_knowledge/ | build.py＋mm_harness.js＋本 md | 新工具 |
