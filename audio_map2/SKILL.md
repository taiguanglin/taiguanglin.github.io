...
6. **「零長度」段（`"zero": true`，2026-09 新增）＝師父沒念這段**：起訖必須嚴格相等
   （`start == end`）、時間欄唯讀、不可點播、勾選即視為已確認。調整前後段落時其邊界
   自動吸附（可穿越連續多個零段）；注入器遇 `zero: true` 直接跳過。人工校正時
   「誤留極短長度的沒念段」就用此勾選修復，不要手動把起訖改成完全相等。
