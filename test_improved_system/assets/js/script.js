document.addEventListener('DOMContentLoaded', function() {
  ...
  function createFloatingTOC() {
    ...
    if (currentChapter.isHomepage) {
      // 首頁：查找現有的目錄結構
      const existingTOC = document.querySelector('.toc, #toc, [class*="table-of-contents"], ol, ul');
      
      if (existingTOC) {
        // 如果找到現有目錄，提取其內容
        const tocLinks = existingTOC.querySelectorAll('a[href], li, dt, dd');
        tocLinks.forEach((item, index) => {
          ...
          } else if (item.tagName === 'LI' || item.tagName === 'DT' || item.tagName === 'DD') {
          ...
          if (text && text.length > 0) {
            const isSubItem = item.tagName === 'DD' || 
                            item.classList.contains('sub') ||
                            item.parentElement && item.parentElement.tagName === 'UL' &&
                            item.parentElement.parentElement && item.parentElement.parentElement.tagName === 'LI';
            
            const levelClass = isSubItem ? ' level-h3' : '';
            const clickAction = href ? 'window.open("' + href + '", "_self")' : '';
            
            tocItems += '<div class="floating-toc-item' + levelClass + '" onclick="' + clickAction + '" style="cursor: pointer;">' + text + '</div>';
          }
        });
      }
      
      // 如果沒有找到現有目錄或目錄為空，創建基本導航
      if (!tocItems) {
        tocItems = 
          '<div class="floating-toc-item">📖 電子書導航</div>' +
          '<div class="floating-toc-item level-h3">使用浮動目錄查看各章節內容</div>' +
          '<div class="floating-toc-item level-h3">點擊頁面中的章節鏈接進行導航</div>';
      }
    } else {
      // 其他頁面：收集頁面內標題
  ...
  // 清空當前文件的所有書籤
  function clearCurrentFileBookmarks() {
    const currentFileBookmarks = getCurrentChapterBookmarks();
    ...
    // 確認對話框
    const confirmed = confirm('確定要清空當前文件的所有 ' + currentFileBookmarks.length + ' 個書籤嗎？此操作無法撤銷。');
    ...
    bookmarksHTML += 
      '<div class="current-chapter-info">' +
        '<div class="current-chapter-header">' +
          ...
          '<button class="clear-bookmarks-btn" data-action="clear-bookmarks" title="清空書籤">🗑️</button>' +
        '</div>' +
  ...
  document.addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains('floating-toc-item')) {
      ...
      // 首頁的目錄項目使用onclick處理跳轉，這裡不需要額外處理
      if (currentChapter.isHomepage && !target) {
        return; // 讓onclick處理器處理跳轉
      }
