...
from utils.image_markup import render_img_tag, alt_from_context
...
def _render_content_piece(piece: str, *, text_tag: str, img_alt: str = "文章配圖") -> str:
    """卡片內一段：一般文字 → ``text_tag``；圖片標記 → 統一 ``<img>``。"""
    if _is_img_marker(piece):
        rel = _img_marker_path(piece)
        return render_img_tag(rel, img_alt)
...
class PDFParser:
    ...
    def _render_img(self, rel_path: str, alt_text: str) -> str:
        """統一 <img> 標記：lazy + 語意 alt + 可得檔案時帶 width/height。"""
        abs_path = None
        if self.image_handler is not None:
            try:
                abs_path = self.image_handler.file_manager.get_file_path(rel_path)
            except Exception:
                abs_path = None
        return render_img_tag(rel_path, alt_text, abs_path)
            ...
            for idx, p in enumerate(paras):
                if _is_img_marker(p):
                    pieces.append(
                        ...
                        + self._render_img(
                            _img_marker_path(p),
                            alt_from_context(paras, idx, fallback="提問配圖"),
                        )
                    )
                else:
            ...
            for idx, p in enumerate(paras):
                if _is_img_marker(p):
                    pieces.append(
                        ...
                        + self._render_img(
                            _img_marker_path(p),
                            alt_from_context(paras, idx, fallback="回答配圖"),
                        )
                    )
                else:
