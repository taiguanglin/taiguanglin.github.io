...
def _base_id(qid: Optional[str]) -> Optional[str]:
    """去掉 IDGenerator 的去重后缀（-2、-3…），返回基底 id。

    生成器对重复出现的同基底 id 追加 ``-{n}`` 后缀；音档映射（audio_map /
    audio_map2 的 ``question_id`` / ``chapter_question_ids`` /
    ``chapter_answer_ids``）冻结的仍是基底 id，查找未中时以基底重试。
    """
    if not qid:
        return qid
...
def _inject_section(section: str, session: dict) -> str:
    ...
    for q_index, qm in enumerate(q_matches, start=1):
        ...
        seg = by_qid.get(qid)
        if seg is None and qid:
            seg = by_qid.get(_base_id(qid))
...
def inject_word_html_from_audio_map2(
    ...
    for i, m in enumerate(q_matches):
        ...
        if seg is None:
            ...
            if seg is None:
                seg = by_qid.get(_base_id(m.group(1)))
