...
def finalize_chapter(
    ...
    content_blocks = wrap_qa_pairs(content_blocks)
...
def wrap_qa_pairs(content_blocks: List[str]) -> List[str]:
    """把相鄰的 question/answer 區塊包進 ``<article class="qa-pair">``。

    語意邊界：螢幕閱讀器可朗讀「文章」數量並逐對導覽問答；視覺不變
    （``qa-pair`` 僅為語意容器，不加樣式）。規則：

    - ``question`` 後緊跟一或多個 ``answer`` → 整組包成一個 article。
    - 單獨出現的 question（如師父連續發帖、無對應提問）→ 不包。
    - ``<hr/>``、標題、其他區塊一律保持在 article 之外（分隔線在對外）。
    """
    out = []
    i = 0
    ...
    while i < n:
        ...
        if block.startswith('<div class="question"'):
            j = i + 1
            while j < n and content_blocks[j].startswith('<div class="answer"'):
                j += 1
            if j > i + 1:
                out.append('<article class="qa-pair">')
                out.extend(content_blocks[i:j])
                out.append('</article>')
                i = j
                continue
        out.append(block)
