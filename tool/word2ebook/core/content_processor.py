...
# bs4 序列化會把屬性名統一轉小寫，SVG 的 camelCase 屬性（目前僅 viewBox）
# 會被改寫成 viewbox 而汙染整份章節 HTML。這裡在序列化輸出後把已知屬性還原。
_CAMEL_CASE_ATTRS = {
    'viewbox': 'viewBox',
}
_CAMEL_CASE_ATTR_RE = re.compile(
    r'\b(' + '|'.join(_CAMEL_CASE_ATTRS) + r')="',
    re.IGNORECASE,
)


def restore_camel_case_attributes(html: str) -> str:
    """還原 bs4 序列化時被小寫化的 camelCase 屬性（如 viewBox）。"""
    return _CAMEL_CASE_ATTR_RE.sub(
        lambda m: _CAMEL_CASE_ATTRS[m.group(1).lower()] + '="', html
    )
...
class ContentProcessor:
    ...
    def extract_search_content(self, html_content: str, base_filename: str) -> Tuple[List[SearchItem], str]:
        ...
        # bs4 會重寫整份 HTML（為了持久化元素 ID），需還原被小寫化的屬性名
        return search_items, restore_camel_case_attributes(str(soup))
