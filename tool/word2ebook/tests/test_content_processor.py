...
from core.content_processor import ContentProcessor, restore_camel_case_attributes
...
class TestCamelCaseAttrRestore:
    """bs4 序列化會把屬性名小寫化；輸出前需還原 camelCase（viewBox）。"""

    def test_restore_viewbox(self):
        assert (
            restore_camel_case_attributes('<svg viewBox="0 0 24 24"><path/></svg>')
            == '<svg viewBox="0 0 24 24"><path/></svg>'
        )

    def test_restore_lowercased_viewbox(self):
        assert (
            restore_camel_case_attributes('<svg viewbox="0 0 24 24"></svg>')
            == '<svg viewBox="0 0 24 24"></svg>'
        )

    def test_plain_html_untouched(self):
        html = '<div class="question" id="q1">內容</div>'
        assert restore_camel_case_attributes(html) == html

    def test_extract_preserves_viewbox_through_bs4(self, processor):
        # bs4 4.15 序列化會把 viewBox 小寫成 viewbox；extract_search_content
        # 的輸出（會被寫回章節 HTML）必須還原原貌。
        html = (
            '<html><body><div class="answer"><div class="answer-meta">'
            '<span class="answerer">Taiguanglin</span></div>'
            '<div class="answer-text">'
            '<svg class="qa-play-icon" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>'
            '回答內容，長度足夠說明。</div></div></body></html>'
        )
        _, updated_html = processor.extract_search_content(html, "01.html")
        assert 'viewBox="0 0 24 24"' in updated_html
        assert 'viewbox=' not in updated_html


class TestNoContextEmission:
    """context 欄位不再輸出（與 content 重複，佔索引體積）。"""

    def test_search_items_have_no_context_payload(self, processor):
        items, _ = processor.extract_search_content(SIMPLE_HTML, "01.html")
        assert len(items) > 0
        for item in items:
            assert item.to_dict().get("context", None) is None
