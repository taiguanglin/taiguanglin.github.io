...
class TestWrapQAPairs:
    def test_question_answer_wrapped_in_article(self):
        blocks = [
            '<h2 id="s">節</h2>',
            '<div class="question" id="q1">Q</div>',
            '<div class="answer" id="a1">A</div>',
            '<hr/>',
        ]
        out = wrap_qa_pairs(blocks)
        html = "\n".join(out)
        assert '<article class="qa-pair">' in html
        assert '</article>' in html
        # article 內含 q 與 a
        start = html.index('<article class="qa-pair">')
        end = html.index('</article>')
        inner = html[start:end]
        assert 'id="q1"' in inner and 'id="a1"' in inner
        # hr 在 article 之外
        assert html.index('<hr/>') > end

    def test_multiple_consecutive_answers_in_one_article(self):
        blocks = [
            '<div class="question" id="q1">Q</div>',
            '<div class="answer" id="a1">A1</div>',
            '<div class="answer" id="a2">A2</div>',
        ]
        out = wrap_qa_pairs(blocks)
        html = "\n".join(out)
        assert html.count('<article class="qa-pair">') == 1
        assert 'id="a2"' in html

    def test_orphan_question_not_wrapped(self):
        blocks = [
            '<div class="question" id="q1">Q</div>',
            '<hr/>',
            '<div class="answer" id="a1">A</div>',
    ...
    def test_finalize_chapter_output_contains_articles(self):
        ...
        toc_items = [(2, "節", "s")]
        finalize_chapter(ch, blocks, toc_items, DEFAULT_SETTINGS)
        assert '<article class="qa-pair">' in ch.content
        assert '</article>' in ch.content
        # QA 計數（regex 掃 div.question）不受 article 包裹影響
        assert ch.qa_count_metadata.get_count_for_anchor("s") == 1
