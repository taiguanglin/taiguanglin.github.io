...
class TestLoadMaps:
    ...
    def test_suffixed_qid_falls_back_to_base(self):
        # PDF 章：IDGenerator 去重後綴 id → 以基底 qid 重試後命中已聆聽段
        section = SAMPLE_SECTION.replace('id="question-aaa"', 'id="question-aaa-2"')
        by_section = {"2025nian-11yue-10ri-guan-wang": _session()}
        out = inject_html(section, by_section)
        assert out.count('class="qa-play qa-play--inline"') == 1
        assert 'data-start="10.000"' in out
