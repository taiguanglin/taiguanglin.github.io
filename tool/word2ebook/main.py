...
from config.settings import Settings, DEFAULT_SETTINGS, Constants
...
class Word2EBookConverter:
    ...
    def _generate_static_assets(self) -> None:
        ...
        if minisearch_js_path.exists():
            with open(minisearch_js_path, 'r', encoding='utf-8') as f:
                minisearch_js_content = f.read()
            self.file_manager.write_file(
                Constants.MINISEARCH_LOCAL_PATH, minisearch_js_content
            )
