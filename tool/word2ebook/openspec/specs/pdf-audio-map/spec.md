...
- GIVEN the ebook's `IDGenerator` de-duplication gave a question block the id
  `question-X-2`, while the audio map froze `question_id: "question-X"`
- WHEN the section is injected
- THEN the question SHALL be matched against the base id `question-X` (exact
  full-id match still takes precedence), so a listened segment still yields
  its inline button
