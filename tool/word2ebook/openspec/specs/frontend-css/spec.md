...
| `04c-qa-audio.css` | QA chapter styles: source banner, per-segment `qa-meta-bar` (number + `.qa-play` button + status badge), `qa-opening`, the bottom floating `qa-player` (seek row with `−5s`/progress/`+5s`, play/pause toggle, Bilibili-style volume control — a pink SVG speaker icon (~3/4 emoji size, `fill: var(--color-primary)`, sound waves hidden and a slash shown when muted) whose popup (numeric 0–100 readout + vertical rotated range slider filled up to `--qa-volume-pct` with the primary colour) appears on hover; a `::before` bridge spans the 10px gap above the button so hovering from button to slider stays continuous), loading states (`.qa-play.loading`, `.qa-play-icon--spinner`, `.qa-player.is-loading`, indeterminate progress pulse) with matching `body.dark-mode` overrides (icon uses `#ff91af` in dark mode); dark-mode variants. Ordered before `05-responsive.css` so its `@media` overrides win. Responsive rules for these live in `05-responsive.css`. Also hosts 講經段落跟播 styles: `.para-track-toggle` (per-lecture follow-play switch inserted next to `.qa-play`, `on` state = gradient fill), `.para-autostop-toggle` (auto-stop-at-paragraph-end switch in the player seek row, reusing `.qa-player-skip` sizing), `.para-block.para-active` (warm-glow background + inset left accent via `box-shadow`, `font-weight: 600`, `transform: scale(1.02)` to avoid reflow) and `.para-block.para-prev` (dimmed), each with `body.dark-mode` variants |
