...
| `04c-qa-audio.css` | QA chapter styles: source banner, per-segment `qa-meta-bar` (number + `.qa-play` button + status badge), `qa-opening`, the bottom floating `qa-player` (seek row with `−5s`/progress/`+5s`, play/pause toggle, Bilibili-style volume control — a pink SVG speaker icon (~3/4 emoji size, `fill: var(--color-primary)`) that toggles a popup with a numeric 0–100 readout and a vertical (rotated) range slider whose track fills up to `--qa-volume-pct` with the primary colour), loading states (`.qa-play.loading`, `.qa-play-icon--spinner`, `.qa-player.is-loading`, indeterminate progress pulse) with matching `body.dark-mode` overrides (icon uses `#ff91af` in dark mode); dark-mode variants. Ordered before `05-responsive.css` so its `@media` overrides win. Responsive rules for these live in `05-responsive.css` |
