...
| `10-search-return.js` | 「回到搜尋結果」：章節頁偵測 `w2eSearchSnapshot`（sessionStorage）有活躍查詢時注入固定按鈕，點擊回 `index.html?q=…#q=…`（避讓浮動層級控制與 QA 播放器）；index 頁持續快照 `{q, scope, displayed, scrollY}` 供還原已顯示筆數與捲動位置 |
| `11-reading-resume.js` | 閱讀位置記憶（`localStorage w2e:readpos`，頁面→捲動比例，LRU 40 頁）：重回同頁且無錨點時頂部提示「回到上次閱讀位置（X%）」；優先消化 `/lang-switch.js` 寫入的 `sessionStorage w2e:langjump`（簡繁雙頁切換原位恢復） |
| `12-bookmarks-manager.js` | 首頁主內容「我的書籤」跨章節管理區塊：可收合、按章節分組（章名＋摘錄＋件數），跳轉／刪除／清空全部（confirm）；沿用 03a 的 localStorage 鍵與 `removeBookmarkById` |
