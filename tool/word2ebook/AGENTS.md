...
main.py
  Word2EBookConverter.convert()
    ...
    3.6 inject_word_chapters()            → audio_map2 (reviewed, chapter_question_ids keyed) → `.qa-play` on Word chapters (no-op if maps absent)
...
| `utils/i18n_utils.py` | `I18nProcessor` — Simplified↔Traditional conversion (t2s → s2twp → 只/隻・發/髮・後/后・裡/里 語境修正 → 異體字標準化，字集語料硬化), filename transforms；`lang-switch.js` 有同源 JS 鏡像層，兩份字集須同步 | 350 |
...
| `10-search-return.js` | 「回到搜尋結果」浮動按鈕已移除（2026-09：快照跨分頁複製不穩、按鈕鮮少出現）。僅保留 index 頁快照 `{q, scope, displayed, scrollY}`（`w2eSearchSnapshot`，sessionStorage）供返回時由 01e 還原 |
