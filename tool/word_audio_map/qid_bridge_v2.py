#!/usr/bin/env python3
"""v2 R2 — bridge thematic question_ids to chrono blocks placed by D1.

Reads  build/questions.json (thematic ebook source of truth)
       build/session_alignment.json
Writes build/qid_bridge_v2.json + build/bridge_conflicts_v2.md
...
def main():
    ...
    chrono = json.loads((TOOL / "build" / "chrono_sessions.json").read_text())
    D1 = json.loads((TOOL / "build" / "docx_audio_map.json").read_text())

    # ---- flatten placed chrono blocks ------------------------------------
    blocks = []
    n_unplaced = 0
    for ch in chrono["chapters"]:
        date = ch["session_date"]
        place = {}
        for run in (D1.get(date) or {}).get("runs", []):
            for sg in run["segments"]:
                place[sg["seq"]] = (run["sid"], sg.get("status", "review"))
        for b in ch["blocks"]:
            if b["seq"] not in place:
                n_unplaced += 1
                continue
            sid, st = place[b["seq"]]
            blocks.append({
                ...
                "sid": sid,
                "seq": b["seq"],
                "asker": b.get("asker_raw") or "",
                "q": py_norm((b.get("q_text") or "")[:400], conv),
                "a": py_norm((b.get("a_text") or "")[:400], conv),
                "status": st,
            })
    blocks.sort(key=lambda b: b["date"])
    print(f"placed blocks={len(blocks)} unplaced={n_unplaced}")
    ...
    (BUILD_DIR / "qid_bridge_v2.json").write_text(
    ...
    (BUILD_DIR / "bridge_conflicts_v2.md").write_text("\n".join(lines), encoding="utf-8")
