...
另有一支**針對剩餘缺口**的腳本（帶 `--apply` 才寫檔、dry-run 預設、
列印每筆待人工複核）：

`fill_resolvable.py` —— 為「有實質問答、但分類檔找不到對應」的段，以
**q_text**（包含關係優先、再 fuzzy ratio ≥ 0.6，並有最短字數護欄，避免
「感恩师父！」這類問候被誤掛多題）補上最相近的分類題。其**故意不用
answer_text**——那正是先前把 57 個 qid 誤掛到一個問候段的來源。適合人工
審核後再 `--apply`。

### 以 `wenda2_ebook` 為準的分段稽核／重切（`split_missing_subquestions.py`）

上面 `apply_resplit.py` 是「用新 parser 重跑、再搬已校對欄位」；本腳本則反過來：
**以 `wenda2_ebook/01–12.html` 的 `<div class="question|answer" id="…">` 為唯一
分段依據**，逐段比對 `audio_map2/*.json` 既有分段，字數不符就依 HTML 重切。

```bash
cd tool/word_audio_map2
.venv/bin/python split_missing_subquestions.py            # dry-run（逐筆列 findings）
.venv/bin/python split_missing_subquestions.py --apply    # 只寫有變動的月份
.venv/bin/python split_missing_subquestions.py --debug 2025-03-15-tieba#18
.venv/bin/python mark_html_diff.py --apply                # 重切後補 html_verbatim
.venv/bin/python validate_resplit.py && .venv/bin/python validate_relink.py
```

判讀方式：把每段的 `q_text`/`answer_text` 去標點後（`[\w一-鿿]`，`kept_stream`）
與 HTML block 對齊，**逐段比較 kept 字數**；差異超出容許（約 block 長度的
8–12%）即認定分段錯誤。判定標籤：

| verdict | 意義 | 動作 |
|---------|------|------|
| `SPLIT` | 一段含多題，字數逐題吻合 HTML | 依 block 邊界切片（時間按字數比例） |
| `MERGE` | 同一 block 的問題被 Word 拆到連續多段（前段 `start=None`） | 合併為一段（信封時間、聯集 qid） |
| `QA-FIX` | 答案文字被 Word parser 留在 `q_text`（qΔ=+N、aΔ=−N） | 依 block 邊界把尾段移入 `answer_text` |
| `LINK-FIX` | 章節對應掛錯（含兩段互換 qid、指向無關問題） | 改寫 `chapter_question_ids` |
| `ALIGN` | 上段答案尾的過渡語其實是下段問題的前綴 | 把文字搬到下一段 |
| `SPLIT-DEGEN` | 邊界對不上（答案比對率過低） | **拒絕切分**，只列報表待人工 |
| `weak` / `LINK-WEAK` / `NO-CLAIM` | 單題但答案側有落差／對應不足／找不到 block | 只列報表 |

重切／合併／QA 修正過的段會標上 `html-resplit: …` 的 `notes`、`status=auto`、
`confidence ≤ 0.5`，並**移除 `meta.lastPlayed`**（時間範圍已變，需重新人工複聽）；
合併偵測會用 `all_groups`（含只比對到答案側的 block），但只有 Q／RB／AONLY
claim 會被實際切片，避免誤切。

> 驗收標準（本工具在 2026 年套用於 2024-02…2025-05 全 14 個月）：
> `validate_relink.py` 的「凍結 qid 零遺失」= 0、spurious 跨 session qid = 0，
> 且**逐 session kept 字元總和不得減少**（split/merge 只在 block 開頭補回共用
> 表頭，故只增不減；若出現淨減少即為邊界漏字）。套用後再跑一次應為 no-op
> （冪等），只剩 4 筆 `SPLIT-DEGEN` 需人工。

### 注意

- **絕對不要**整份重建 `audio_map2/*.json` 而丟掉人工校對的 `start/end` 與
  `meta.lastPlayed`；重分段一律走內容比對遷移（`apply_resplit.py`）。
  （`split_missing_subquestions.py` 是唯一例外：它依 HTML 改動了段邊界，
  因此會主動移除**被改動段**的 `lastPlayed`，使其回到待複聽狀態。）
- 有「最後播放」(`meta.lastPlayed`) 紀錄的段**任何腳本都不應改動**——那是
  音檔時間人工校對的完成判定；前述 fill 腳本只處理 `lastPlayed` 為空的段。
