...
from config import SHARED_ASSETS_DIR, SHARED_ASSET_FILES
...
def check_files(out):
    need = (
        ...
        + [os.path.join("assets", "css", "books.css"),
           os.path.join("assets", "js", "w2e-config.js")]
    )
    missing = [f for f in need if not os.path.exists(os.path.join(out, f))]
    for f in missing:
        issue("bad", out, "缺少檔案：%s" % f)
    # 共用資產不在 ebook/ 底下，而是引用 wenda2_ebook 的產物
    for p in SHARED_ASSET_FILES:
        if not os.path.exists(os.path.join(SHARED_ASSETS_DIR, p)):
            issue("bad", out, "缺少共用資產：wenda2_ebook/assets/%s" % p)
...
def local_target(out, ref, cur_file):
    ...
    if not path:
        return None
    if path.startswith("/"):
        # 站點根絕對路徑（/lang-switch.js、/audio/...）：站點根 = repo 根。
        # 回傳 ../ 形式 → FS 檢查走 REPO_ROOT 候選、HTTP 檢查走站點根 URL。
        return "../" + path.lstrip("/")
...
_HEADING_BODY_RE = re.compile(
    r'<(h[234]) id="([^"]+)">([^<]*)'
    # 講次播放鈕注入在標題與計數 span 之間，計數比對需容忍它
    r'(?:<button class="qa-play"[^>]*>.*?</button>)?'
    r'<span class="chapter-qa-count">\((\d+)\)</span>'
)
...
def check_counts(out, rel, html, p):
    ...
    if not m:
        return  # 首頁沒有 h1 計數
    total = int(m.group(1))
    # 正文標題：tag id >text[(play button)]count</tag（與 _HEADING_BODY_RE 同源）
    body_counts = {}
    for mm in _HEADING_BODY_RE.finditer(html):
