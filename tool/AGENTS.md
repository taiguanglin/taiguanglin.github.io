...
| `tool/books_knowledge/` | 坐禪與講經知識頁生成器（坐禪＋講經九書）。`build.js`＝重點知識頁（按書分冊、91 名詞卡＋深連結）；`build_full.js`＝知識庫全檔頁（64 名詞全檔按主幹分組＋書冊分布＋高頻詞次數，比照 wenda2 知識庫全檔）。SoT：`mindmap.html` 內嵌資料（slice＋eval-IIFE 抽取，含 BOOKS／TERM_COUNTS）；改名詞後重跑兩支同步，名詞對不到書時 build 直接失敗。 | `mindmap.html` → `books_knowledge.html` / `books_knowledge_full.html` | `README.md` |
